/**
 * EventCard - Server Component displaying event summary
 * Links to event detail/registration page
 */
  ...
  return (
        <Link href={`/events/${event.id}`} className="group block">
              <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-6 hover:bg-white/20 transition-all duration-200 hover:scale-105 hover:shadow-xl cursor-pointer">
                ...
                {event.description && (
                    <p className="text-purple-200 text-sm mb-4 line-clamp-2">{event.description}</p>p>
                      )}
              
                      <div className="space-y-2 mb-4">
                                <div className="flex items-center gap-2 text-purple-300 text-sm">
                                            <MapPin className="w-4 h-4 flex-shrink-0" />
                                            <span className="truncate">{event.venue}</span>span>
                                </div>div>
                                <div className="flex items-center gap-2 text-purple-300 text-sm">
                                            <Users className="w-4 h-4 flex-shrink-0" />
                                            <span>{event.capacity} seats</span>span>
                                </div>div>
                      </div>div>
